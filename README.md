...
```bash
game-night$ sam build --use-container
```
...
```bash
game-night$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
game-night$ sam local start-api
game-night$ curl http://localhost:3000/
```
...
```bash
game-night$ sam logs -n HelloWorldFunction --stack-name game-night --tail
```
...
```bash
game-night$ pip install -r tests/requirements.txt --user
# unit test
game-night$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
game-night$ AWS_SAM_STACK_NAME="game-night" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "game-night"
```
